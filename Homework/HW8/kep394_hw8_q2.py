...
#Question 2c) A - For the runtime of the function implemented in section a, the program runs the for loop for O(n).
#Because the insert method increments each time, the insert runs in O(n). Because the insertion takes place within
#the for loop, the function runs in a total time of O(n) * O(n) = O(n^2)
# B - For the runtime of the function implemented in section b, add_items works by recursively calling a range of numbers,
# only working with each number once, meaning O(n),  and inserts it to be balanced which is O(h). Because O(h) = O(log(n)),
# the runtime of the function is O(n) * O(log(n)) = O(nlog(n))
